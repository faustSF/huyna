...
int main()
{
	...
	int x0 = (W / 2);
	int y0 = (H / 2);
	RenderWindow window(sf::VideoMode(W, H), "Grafik");
	SFML::Init(window);
	int o1 = -10;
	int o2 = 15;
	float c = 100;
	...
	Texture t;
	t.loadFromFile("D:/Grafik/Coordinate plane.png");
	Sprite pic(t);
	//int j = 3;
	//int circleSegments = 100;
	//float circleColor[3] = { (float)204 / 255, (float)77 / 255, (float)5 / 255 };
	CircleShape Lpoint(circleRadius);
	CircleShape& point = Lpoint;
	...
	RectangleShape* OsX = new RectangleShape(Vector2f(W*2, 1*(sc/33)));
	OsX->setFillColor(Color::Black);
	OsX->setPosition(y0*(-1),x0);

	RectangleShape* OsY = new RectangleShape(Vector2f(1 * (sc / 33), H*2));

	OsY->setFillColor(Color::Black);
	OsY->setPosition(y0, (-1)*x0);
	int anim = 0;
	...
	while (window.isOpen())
	{
		...
		for (int i = 0; i < 40; i++) {
			...
			if (i < 20) {
				...
				if (i < 30)
					line[i].setPosition(x0+10 , y0 + (i - 30) * (-1)*sc);
				else
					line[i].setPosition(x0+10 , y0 + (i - 29) * (-1)*sc);
			}
		}
